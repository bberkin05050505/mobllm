...
class Workspace(object):
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.logger.info(f"Randomly sampled {self.num_test_pts_m} test points.")
        ...
        if cfg.experiment.function.train_points.generate_points:
            ...
            self.test_points = self.generate_points(self.true_function, self.min_test_points, self.max_test_points, self.num_test_pts_m, random_points=False)
        ...
        if len(self.current_functions.functions) < self.num_to_sample_b:
            ...
            if len(self.current_functions.functions) == 0:
                ...
                raise Exception("No seed functions generated. Exiting.")
        else:
    ...
    def get_seed_prompt(self) -> str:
        """
        Returns the seed generation prompt.
        """
        if self.seed_prompt is None:
            ...
            prompt = prompt.format(points=utils.array_to_string(self.train_points, self.num_digits),        num_variables=self.num_variables, variables_list=[f"x{i+1}" for i in range(self.num_variables)])
    ...
    def generate_seed_functions(self) -> Tuple[Dict[str, Any], float]:
        ...
        prompt, img_path = self.get_seed_prompt()
    ...
    def update_results(self, iteration:int, best_expr, best_func, last_func, start_time, frames) -> None:
        # Update results
        ...
        # Compute R2 values
        train_points = utils.string_to_array(self.all_train_points)
        test_points = self.test_points
        r2_train, r2_test, r2_all = self.SR.get_R2_scores(best_func, train_points, test_points)
        ...
        if self.save_video:
            if last_func is not None:
                frame, ax = self.plotter.record_frame(best_func, last_func, r2_test, self.true_function, iteration, plot_true=True)
                if self.save_frames:
    ...
    def run(self) -> int:
        ...
        # Update the plotters training points to include the newly sampled points
